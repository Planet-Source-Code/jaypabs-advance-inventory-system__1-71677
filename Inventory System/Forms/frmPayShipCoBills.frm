...
Begin VB.Form frmPayShipCoBills 
   BackColor       =   &H00808000&
   BorderStyle     =   0  'None
   Caption         =   "Form1"
   ClientHeight    =   7245
   ...
   Begin VB.CommandButton btnRemove 
      Height          =   275
      Left            =   300
      Picture         =   "frmPayShipCoBills.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   17
      ToolTipText     =   "Remove"
      Top             =   3630
   ...
   Begin VB.Frame PaymentOption 
      ...
      Begin VB.ComboBox cbPT 
         Height          =   315
         ItemData        =   "frmPayShipCoBills.frx":01B2
         Left            =   2880
         List            =   "frmPayShipCoBills.frx":01BC
         Style           =   2  'Dropdown List
         TabIndex        =   1
         Top             =   570
         Width           =   1635
      End
      Begin ctrlNSDataCombo.NSDataCombo nsdPOID 
         Height          =   315
         Left            =   240
         TabIndex        =   9
         Top             =   570
   ...
   End
End
Attribute VB_Name = "frmPayShipCoBills"
...
Private Sub btnAdd_Click()
    If nsdPOID.Text = "" Or cbPT.Text = "" Or txtAmount.Text = "" Then nsdPOID.SetFocus: Exit Sub
    ...
    CurrRow = getFlexPos(Grid, 1, nsdPOID.Text)
    ...
    With Grid
        If CurrRow < 0 Then
            ...
            If .Rows = 2 And .TextMatrix(1, 1) = "" Then
                .TextMatrix(1, 1) = nsdPOID.Text
                .TextMatrix(1, 2) = cbPT.Text
                .TextMatrix(1, 3) = txtAmount.Text
                .TextMatrix(1, 7) = IIf(nsdPOID.BoundText = "", nsdPOID.Tag, nsdPOID.BoundText)
            ...
            Else
                .Rows = .Rows + 1
                .TextMatrix(.Rows - 1, 1) = nsdPOID.Text
                .TextMatrix(.Rows - 1, 2) = cbPT.Text
                .TextMatrix(.Rows - 1, 3) = txtAmount.Text
                .TextMatrix(.Rows - 1, 7) = IIf(nsdPOID.BoundText = "", nsdPOID.Tag, nsdPOID.BoundText)
        ...
        Else
            If MsgBox("Ref No already added. Do you want to replace it?", vbQuestion + vbYesNo) = vbYes Then
                ...
                .TextMatrix(CurrRow, 1) = nsdPOID.Text
                .TextMatrix(CurrRow, 2) = cbPT.Text
                .TextMatrix(CurrRow, 3) = txtAmount.Text
                .TextMatrix(CurrRow, 6) = nsdBank.BoundText
                .TextMatrix(CurrRow, 7) = IIf(nsdPOID.BoundText = "", nsdPOID.Tag, nsdPOID.BoundText)
...
On Error GoTo err
    ...
    PaymentPK = getIndex("Shipping_Guide_Payments")
   
    'Save account to Customer's Ledger
    Dim RSPayments As New Recordset

    RSPayments.CursorLocation = adUseClient
    RSPayments.Open "SELECT * FROM Shipping_Guide_Payments WHERE PaymentID=" & 0, CN, adOpenStatic, adLockOptimistic
    ...
    RSLedger.Open "SELECT * FROM Shipping_Company_Ledger WHERE LedgerID=" & 0, CN, adOpenStatic, adLockOptimistic
    ...
    frmShipCoBalance.RefreshRecords
...
Private Sub nsdPOID_Change()
    If blnNew = True Then Exit Sub
    
    txtAmount.Text = toMoney(nsdPOID.getSelValueAt(3))
...
Private Sub InitNSD()
    ...
    With nsdPOID
        .ClearColumn
        .AddColumn "PO ID", 1300.89
        .AddColumn "Company", 1994.89
        .AddColumn "Balance", 1264.88
        
        .Connection = CN.ConnectionString
        .sqlFields = "POID,ShippingCompany,Balance"
        .sqlTables = "qry_ShipCo_Balance_Per_Forwarder"
        .sqlwCondition = "ShippingCompanyID=" & PK
        .sqlSortOrder = "POID ASC"
...
Private Sub DeleteItems()
    ...
    RSLedger.CursorLocation = adUseClient
    RSLedger.Open "SELECT * FROM Shipping_Company_Ledger WHERE PaymentID='" & PaymentPK & "'", CN, adOpenStatic, adLockOptimistic
        ...
        While Not RSLedger.EOF
            ...
            With Grid
                If CurrRow < 0 Then
                    ...
                    DelRecwSQL "Shipping_Company_Ledger", "LedgerID", "", True, RSLedger!LedgerID
                End If
            End With
            RSLedger.MoveNext
        Wend
