...
Begin VB.Form frmForwardersReceive 
   ...
   End
End
Attribute VB_Name = "frmForwardersReceive"
...
Dim rsForwarders As New Recordset
...
Public Sub CommandPass(ByVal srcPerformWhat As String)
    ...
    Select Case srcPerformWhat
        Case "New"
            frmForwardersGuideAE.State = adStateAddMode
            frmForwardersGuideAE.show vbModal
        Case "Edit"
            If lvList.ListItems.Count > 0 Then
                If isRecordExist("Forwarders_Receive", "ForwarderReceiveID", CLng(LeftSplitUF(lvList.SelectedItem.Tag))) = False Then
                ...
                Else
                    With frmForwardersReceiveAE
                        ...
                        Dim ForwarderReceiveID As Long
                        
                        ForwarderReceiveID = CLng(LeftSplitUF(lvList.SelectedItem.Tag))
                        
                        blnLocked = getValueAt("SELECT Locked FROM Forwarders_Receive WHERE ForwarderReceiveID=" & ForwarderReceiveID, "Locked")
                        ...
                        CN.Execute "UPDATE Forwarders_Receive SET Locked = True WHERE ForwarderReceiveID = " & ForwarderReceiveID
                        
                        blnStatus = getValueAt("SELECT ForwarderReceiveID,Status FROM Forwarders_Receive WHERE ForwarderReceiveID=" & ForwarderReceiveID, "Status")
                        ...
                        .ForwarderReceivePK = ForwarderReceiveID
                        .show vbModal
                        
                        CN.Execute "UPDATE Forwarders_Receive SET Locked = False WHERE ForwarderReceiveID = " & ForwarderReceiveID
                    End With
                End If
            End If
        Case "Search"
...
Private Sub Active()
    With MAIN
        .tbMenu.Buttons(3).Visible = False
        .tbMenu.Buttons(6).Visible = False
...
Private Sub Form_Load()
    ...
    With SQLParser
        .Fields = "ShippingCompany, ShippingGuideNo, PONo, DeliveryDate, QtyReturned, ForwarderReceiveID"
        .Tables = "qry_Forwarders_Receive"
        .SortOrder = "DeliveryDate DESC"
        
        .SaveStatement
    End With
    
    rsForwarders.CursorLocation = adUseClient
    rsForwarders.Open SQLParser.SQLStatement, CN, adOpenStatic, adLockReadOnly
    
    With RecordPage
        .Start rsForwarders, 75
        FillList 1
    End With
...
Private Sub FillList(ByVal whichPage As Long)
    ...
    Me.Enabled = False
    Call pageFillListView(lvList, rsForwarders, RecordPage.PageStart, RecordPage.PageEnd, 16, 2, False, True, , , , "ForwarderReceiveID")
    Me.Enabled = True
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set frmForwardersReceive = Nothing
End Sub
