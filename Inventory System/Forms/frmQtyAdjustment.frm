...
Begin VB.Form frmQtyAdjustment 
   Caption         =   "Qty Adjustment History"
   ClientHeight    =   6675
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   10035
   ...
   End
End
Attribute VB_Name = "frmQtyAdjustment"
...
Public Sub CommandPass(ByVal srcPerformWhat As String)
    ...
    Select Case srcPerformWhat
        Case "New"
            frmQtyAdjustmentAE.State = adStateAddMode
            frmQtyAdjustmentAE.show vbModal
        Case "Edit"
            If lvList.ListItems.Count > 0 Then
                If isRecordExist("Qty_Adjustment", "QtyAdjustmentID", CLng(LeftSplitUF(lvList.SelectedItem.Tag))) = False Then
                    MsgBox "This record has been removed by other user.Click 'OK' button to refresh the records.", vbExclamation
                    RefreshRecords
                    Exit Sub
                Else
                    With frmQtyAdjustmentAE
                        Dim blnStatus As Boolean
                        
                        blnStatus = getValueAt("SELECT QtyAdjustmentID,Status FROM Qty_Adjustment WHERE QtyAdjustmentID=" & CLng(LeftSplitUF(lvList.SelectedItem.Tag)), "Status")
        ...
        Case "Delete"
            If lvList.ListItems.Count > 0 Then
                ...
                Else
                    ...
                    If ANS = vbYes Then
                        DelRecwSQL "Qty_Adjustment", "QtyAdjustmentID", "", True, CLng(LeftSplitUF(lvList.SelectedItem.Tag))
                        RefreshRecords
...
Private Sub Form_Load()
    ...
    With SQLParser
        .Fields = "Reason, Date, Status_Alias,  QtyAdjustmentID"
        .Tables = "qry_Qty_Adjustment"
        .SortOrder = "Date DESC"
        
        .SaveStatement
    End With
    If RSQtyAdj.State = 1 Then RSQtyAdj.Close
    RSQtyAdj.CursorLocation = adUseClient
    RSQtyAdj.Open SQLParser.SQLStatement, CN, adOpenStatic, adLockReadOnly
...
Private Sub FillList(ByVal whichPage As Long)
    ...
    Me.Enabled = False
    Call pageFillListView(lvList, RSQtyAdj, RecordPage.PageStart, RecordPage.PageEnd, 15, 2, False, True, , , , "QtyAdjustmentID")
    Me.Enabled = True
