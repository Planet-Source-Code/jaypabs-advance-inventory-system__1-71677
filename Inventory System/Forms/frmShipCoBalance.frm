...
Begin VB.Form frmShipCoBalance 
   Caption         =   "Shipping Company w/ balance"
   ClientHeight    =   6960
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   9000
   ...
   End
End
Attribute VB_Name = "frmShipCoBalance"
    ...
    Select Case srcPerformWhat
        Case "Edit"
            If lvList.ListItems.Count > 0 Then
                If isRecordExist("Shipping_Company", "ShippingCompanyID", CLng(LeftSplitUF(lvList.SelectedItem.Tag))) = False Then
                    MsgBox "This record has been removed by other user.Click 'OK' button to refresh the records.", vbExclamation
                    RefreshRecords
                    Exit Sub
                Else
                    With frmPayShipCoBills
                        '.State = adStateEditMode
                        .PK = CLng(LeftSplitUF(lvList.SelectedItem.Tag))
                        .POID = lvList.SelectedItem
                        .strForwarder = lvList.SelectedItem.SubItems(1)
                        .blnNew = True
                        .TotalAmount = lvList.SelectedItem.SubItems(2)
        ...
        Case "Delete"
            If lvList.ListItems.Count > 0 Then
                If isRecordExist("Vendors", "ShippingCompanyID", CLng(LeftSplitUF(lvList.SelectedItem.Tag))) = False Then
                    MsgBox "This record has been removed by other user.Click 'OK' button to refresh the records.", vbExclamation
                    RefreshRecords
                    Exit Sub
                Else
                    ...
                    If ANS = vbYes Then
                        DelRecwSQL "Vendors", "ShippingCompanyID", "", True, CLng(LeftSplitUF(lvList.SelectedItem.Tag))
                        RefreshRecords
...
Private Sub Form_Activate()
    HighlightInWin Me.Name: MAIN.ShowTBButton "fttftft"
    Active
End Sub
...
Private Sub Active()
    With MAIN
        .tbMenu.Buttons(4).Caption = "Payment"
    End With
End Sub
...
Private Sub Form_Load()
    ...
    With SQLParser
        .Fields = "POID, ShippingCompany, Balance, ShippingCompanyID"
        .Tables = "qry_ShipCo_Balance_Per_Forwarder"
        .SortOrder = "POID ASC"
        
        .SaveStatement
    End With
    
    If rsShipCo.State = 1 Then rsShipCo.Close
    rsShipCo.CursorLocation = adUseClient
    rsShipCo.Open SQLParser.SQLStatement, CN, adOpenStatic, adLockReadOnly
...
Private Sub FillList(ByVal whichPage As Long)
    ...
    Me.Enabled = False
    Call pageFillListView(lvList, rsShipCo, RecordPage.PageStart, RecordPage.PageEnd, 15, 2, False, True, , , , "ShippingCompanyID")
    Me.Enabled = True
