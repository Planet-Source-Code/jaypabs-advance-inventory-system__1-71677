...
Private Sub DisplayForEditing()
    ...
    With rs
        txtEntry(0).Text = .Fields("Bank")
        txtEntry(1).Text = .Fields("Branch")
        txtEntry(2).Text = .Fields("AccountNo")
        txtEntry(3).Text = .Fields("AccountName")
    End With
    txtEntry(0).SetFocus
    Exit Sub
errHandler:
...
On Error GoTo erR
    ...
    With rs
        .Fields("Bank") = txtEntry(0).Text
        .Fields("Branch") = txtEntry(1).Text
...
Private Sub Form_Load()
    ...
    rs.Open "SELECT * FROM Banks WHERE BankID = " & PK, CN, adOpenStatic, adLockOptimistic
