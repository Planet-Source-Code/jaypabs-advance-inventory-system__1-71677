...
'                        .show vbModal
                ...
                Else
                    ...
                    Else
                        ...
                        If ANS = vbYes Then
                            ...
                            rsD.Open "SELECT * FROM tbl_AR_PaymentHistory WHERE VCFK=" & CLng(LeftSplitUF(lvList.SelectedItem.Tag)) & " ORDER BY ReceiptBatchID ASC", CN, adOpenStatic, adLockOptimistic
                            CN.BeginTrans
                            If rsD.RecordCount > 0 Then
                                ...
                                While Not rsD.EOF
                                    If Len(rsD![InvoiceFK]) > 0 Then
                                        ...
                                        tA = toNumber(getValueAt("SELECT ReceiptBatchID,AmountPaid FROM tbl_AR_Invoice WHERE ReceiptBatchID=" & toNumber(rsD![InvoiceFK]), "AmountPaid"))
                                        tA = tA - rsD![Amount]
                                        'Restore amount paid
                                        ChangeValue CN, "tbl_AR_Invoice", "AmountPaid", tA, True, "WHERE ReceiptBatchID=" & toNumber(rsD![InvoiceFK])
                                        ChangeValue CN, "tbl_AR_Invoice", "Paid", "N", False, "WHERE ReceiptBatchID=" & toNumber(rsD![InvoiceFK])
                            ...
                            End If
                            'Remove the main
                            DelRecwSQL "Collection", "ReceiptBatchID", "", True, CLng(LeftSplitUF(lvList.SelectedItem.Tag))
                            CN.CommitTrans
                            'Clear variables
                            rsD.Close
                            Set rsD = Nothing
        ...
        Case "Print"
            ...
            With frmReports
                ...
                .strWhere = "{qry_rpt_Sales_Report.ReceiptBatchID} = " & CLng(LeftSplitUF(lvList.SelectedItem.Tag))
...
Private Sub Form_Activate()
    HighlightInWin Me.Name: MAIN.ShowTBButton "ftttttt"
    Active
End Sub
...
Private Sub Form_Load()
    ...
    With SQLParser
        ...
        .SortOrder = "ReceiptBatchID DESC"
        
        .SaveStatement
    End With
    
    RSCollection.CursorLocation = adUseClient
    RSCollection.Open SQLParser.SQLStatement, CN, adOpenStatic, adLockReadOnly
    
    With RecordPage
        .Start RSCollection, 75
        FillList 1
    End With
...
Private Sub FillList(ByVal whichPage As Long)
    ...
    Me.Enabled = False
    Call pageFillListView(lvList, RSCollection, RecordPage.PageStart, RecordPage.PageEnd, 16, 2, False, True, , , , "ReceiptBatchID")
    Me.Enabled = True
